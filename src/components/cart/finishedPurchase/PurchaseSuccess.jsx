...
const PurchaseSuccess = ({ onClose, orderDetails }) => {
  ...
  const sendOrderToWhatsApp = () => {
    ...
    const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`; // Monta a URL do WhatsApp com o número de telefone e a mensagem codificada.
  ...
  return (
    <Modal onClose={onClose} isDarkMode={isDarkMode}> {/* Renderiza o componente Modal passando as props onClose e isDarkMode. */}
      <div className={classes.modal}> {/* Div que representa o conteúdo do modal com estilização específica. */}
        <p className={classes.title}>Parabéns!</p> {/* Título do modal com uma mensagem de sucesso. */}
        <p>Sua compra foi finalizada.</p> {/* Mensagem de confirmação de que a compra foi finalizada. */}
        <p>Seu pedido chegará em breve.</p> {/* Mensagem informando ao usuário que o pedido está a caminho. */}
        <WrapperButton className={classes.button} onClick={sendOrderToWhatsApp}> {/* Botão estilizado para enviar o pedido via WhatsApp. */}
          Finalizar Compra
        </WrapperButton>
      </div>
